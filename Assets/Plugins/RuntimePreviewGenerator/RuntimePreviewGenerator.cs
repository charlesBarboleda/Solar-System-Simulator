...
public static class RuntimePreviewGenerator
{
	private class CameraSetup
	{
		...
		// HDRP
		private bool hadHDData;
		private FrameSettings frameSettings;
		private FrameSettingsOverrideMask frameSettingsMask;
		...
		public void GetSetup(Camera camera)
		{
			...
			hadHDData = hdData != null;

			if (hadHDData)
			{
				frameSettings = hdData.renderingPathCustomFrameSettings;
				frameSettingsMask = hdData.renderingPathCustomFrameSettingsOverrideMask;
				antialiasing = hdData.antialiasing;
			}
		}
		...
		public void ApplySetup(Camera camera)
		{
			...
			// Restore HDRP settings
			HDAdditionalCameraData hdData = camera.GetComponent<HDAdditionalCameraData>();

			if (hadHDData && hdData != null)
			{
				hdData.renderingPathCustomFrameSettings = frameSettings;
				hdData.renderingPathCustomFrameSettingsOverrideMask = frameSettingsMask;
				hdData.antialiasing = antialiasing;
			}
		}
	}
	...
	private static Color m_backgroundColor = new Color(0.12f, 0.12f, 0.12f, 1f);
	...
	public static Texture2D GenerateModelPreview(Transform model, int width = 64, int height = 64, bool shouldCloneModel = false, bool shouldIgnoreParticleSystems = true, bool isStarBody = false)
	{
		return GenerateModelPreviewInternal(model, null, null, width, height, shouldCloneModel, shouldIgnoreParticleSystems, isStarBody: isStarBody);
	}
...
#if UNITY_2018_2_OR_NEWER
	private static Texture2D GenerateModelPreviewInternal(Transform model, Shader shader, string replacementTag, int width, int height, bool shouldCloneModel, bool shouldIgnoreParticleSystems, Action<Texture2D> asyncCallback = null, bool isStarBody = false)
#else
...
#endif
	{
		...
		previewObject.GetPositionAndRotation(out Vector3 prevPos, out Quaternion prevRot);
		...
		try
		{
			...
			CalculateCameraPosition(renderCamera, previewBounds, m_padding, isStarBody);
			renderCamera.transform.LookAt(previewBounds.center, previewObject.up);
			if (m_previewLight != null)
				m_previewLight.transform.position = renderCamera.transform.position;
			...
			try
			{
				...
				if (!shader) renderCamera.Render();
				else renderCamera.RenderWithShader(shader, replacementTag ?? string.Empty);
		...
		finally
		{
			...
			{
				if (!wasActive) previewObject.gameObject.SetActive(false);
				if (!isStatic) previewObject.SetPositionAndRotation(prevPos, prevRot);
	...
	public static void CalculateCameraPosition(Camera camera, Bounds bounds, float padding = 0f, bool isStarBody = false)
	{
	...
	private static Light m_previewLight;
	private static void SetupCamera(bool isStarBody = false)
	{
		...
		else renderCamera = InternalCamera;

		renderCamera.backgroundColor = m_backgroundColor;
		renderCamera.orthographic = m_orthographicMode;
		renderCamera.clearFlags = CameraClearFlags.SolidColor;

		// HDRP: Disable post processing for preview rendering
		if (!renderCamera.TryGetComponent<HDAdditionalCameraData>(out var hdData))
		{
			hdData = renderCamera.gameObject.AddComponent<HDAdditionalCameraData>();
		}

		hdData.customRenderingSettings = true;

		// Clear previous HD data
		hdData.clearColorMode = HDAdditionalCameraData.ClearColorMode.Color;
		hdData.backgroundColorHDR = m_backgroundColor;

		hdData.hasPersistentHistory = isStarBody;

		// Post-processing
		hdData.renderingPathCustomFrameSettingsOverrideMask.mask[(int)FrameSettingsField.Postprocess] = true;
		hdData.renderingPathCustomFrameSettings.SetEnabled(FrameSettingsField.Postprocess, isStarBody);

		// Custom post-processes
		hdData.renderingPathCustomFrameSettingsOverrideMask.mask[(int)FrameSettingsField.CustomPostProcess] = true;
		hdData.renderingPathCustomFrameSettings.SetEnabled(FrameSettingsField.CustomPostProcess, isStarBody);

		// Anti-aliasing
		hdData.antialiasing = isStarBody ? HDAdditionalCameraData.AntialiasingMode.TemporalAntialiasing : HDAdditionalCameraData.AntialiasingMode.None;

		// Motion vectors
		hdData.renderingPathCustomFrameSettingsOverrideMask.mask[(int)FrameSettingsField.MotionVectors] = true;
		hdData.renderingPathCustomFrameSettings.SetEnabled(FrameSettingsField.MotionVectors, isStarBody);

		// Object motion vectors
		hdData.renderingPathCustomFrameSettingsOverrideMask.mask[(int)FrameSettingsField.ObjectMotionVectors] = true;
		hdData.renderingPathCustomFrameSettings.SetEnabled(FrameSettingsField.ObjectMotionVectors, isStarBody);

		// Transparent motion vectors
		hdData.renderingPathCustomFrameSettingsOverrideMask.mask[(int)FrameSettingsField.TransparentsWriteMotionVector] = true;
		hdData.renderingPathCustomFrameSettings.SetEnabled(FrameSettingsField.TransparentsWriteMotionVector, isStarBody);

		// Dynamic resolution
		hdData.allowDynamicResolution = isStarBody;

		// Dithering / temporal effects
		hdData.taaSharpenStrength = isStarBody ? 1f : 0f;

		if (m_previewLight == null || !m_previewLight)
		{
			GameObject lightGO = new("ModelPreviewLight")
			{
				hideFlags = HideFlags.HideAndDontSave
			};
