            ...
            case InputType.Calendar:
                {
                    ...
                    {
                        UIMessage.Instance.NewFadingMessage(MessageType.Error, $"[Stop Time] Calendar date out of range", 20f);
                        return false;
                    }
    ...
    public bool IsValidInput()
    {
        ...
        switch (inputType)
        {
            case InputType.Calendar:
                ...
                {
                    UIMessage.Instance.NewFadingMessage(MessageType.Error, $"[Stop Time] Invalid Year input '{_yearInputField.text}'", 20f);
                    _yearInvalidInput.SetActive(true);
                    return false;
                }
                if (!HorizonsAPIParameters.IsValidMonth(_monthInputField.text, out int m))
                {
                    UIMessage.Instance.NewFadingMessage(MessageType.Error, $"[Stop Time] Invalid Month input '{_monthInputField.text}'", 20f);
                    _monthInvalidInput.SetActive(true);
                    return false;
                }
                if (!HorizonsAPIParameters.IsValidDay(_dayInputField.text, m, y, out _))
                {
                    UIMessage.Instance.NewFadingMessage(MessageType.Error, $"[Stop Time] Invalid Day input '{_dayInputField.text}'", 20f);
                    _dayInvalidInput.SetActive(true);
                    return false;
                }
                if (!HorizonsAPIParameters.IsValidHour(_hourInputField.text, out _))
                {
                    UIMessage.Instance.NewFadingMessage(MessageType.Error, $"[Stop Time] Invalid Hour input '{_hourInputField.text}'", 20f);
                    _hourInvalidInput.SetActive(true);
                    return false;
                }
                if (!HorizonsAPIParameters.IsValidMinute(_minuteInputField.text, out _))
                {
                    UIMessage.Instance.NewFadingMessage(MessageType.Error, $"[Stop Time] Invalid Minute input '{_minuteInputField.text}'", 20f);
                    _minuteInvalidInput.SetActive(true);
                    return false;
                }
                if (!HorizonsAPIParameters.IsValidSecond(_secondInputField.text, out _))
                {
                    UIMessage.Instance.NewFadingMessage(MessageType.Error, $"[Stop Time] Invalid Second input '{_secondInputField.text}'", 20f);
            ...
            case InputType.Julian:
                ...
                {
                    UIMessage.Instance.NewFadingMessage(MessageType.Error, $"[Stop Time] Invalid Julian input '{_julianInputField.text}'", 20f);
            ...
            case InputType.ModifiedJulian:
                ...
                {
                    UIMessage.Instance.NewFadingMessage(MessageType.Error, $"[Stop Time] Invalid Modified Julian input '{_mJulianInputField.text}'", 20f);
